...
// HttpOptions is used for Web.state, which is a superset and run time equivalent
...
bool HttpCheckPriviledgedAccess(bool only_for_admin, bool autorequestauth)
{
  ...
  if (only_for_admin && (HTTP_USER == Web.state)) {
    HandleRoot();
    return false;
  }
  ...
  if (autorequestauth) {
    if ((HTTP_MANAGER_RESET_ONLY == Web.state) || // in special setup mode: say it's OK. TODO: why??
...
void HandleRoot(void)
{
  ...
  if (WifiIsInManagerMode()) {
    ...
    if ((Settings.web_password[0] != 0) && !(WebServer->hasArg("USER1")) && !(WebServer->hasArg("PASS1")) && HTTP_MANAGER_RESET_ONLY != Web.state) {
      ...
      if (!(Settings.web_password[0] != 0) || (((WebServer->arg("USER1") == WEB_ADMINNAME ) && (WebServer->arg("PASS1") == Settings.web_password )) || HTTP_MANAGER_RESET_ONLY == Web.state)) {
...
void HandleHttpCommand(void)
{
  ...
  if (Settings.web_password[0] != 0) {
    ...
    if (!(!strcmp(tmp1, WEB_ADMINNAME) && !strcmp(tmp2, Settings.web_password))) { valid = false; }
  }
...
const char kWebCommands[] PROGMEM = "|"  // No prefix
  ...
  D_CMND_EMULATION "|"
#endif
  D_CMND_WEBSERVER "|" D_CMND_WEBPASSWORD "|" D_CMND_USERPASSWORD "|" D_CMND_WEBLOG "|" D_CMND_WEBREFRESH "|" D_CMND_WEBSEND "|" D_CMND_WEBCOLOR "|" D_CMND_WEBSENSOR;

void (* const WebCommand[])(void) PROGMEM = {
#ifdef USE_EMULATION
  &CmndEmulation,
#endif
  &CmndWebServer, &CmndWebPassword, &CmndUserPassword, &CmndWeblog, &CmndWebRefresh, &CmndWebSend, &CmndWebColor, &CmndWebSensor };
...
void CmndUserPassword(void)
{
  if ((XdrvMailbox.data_len > 0) && (XdrvMailbox.data_len < sizeof(Settings.user_password))) {
    strlcpy(Settings.user_password, (SC_CLEAR == Shortcut()) ? "" : (SC_DEFAULT == Shortcut()) ? USER_PASSWORD : XdrvMailbox.data, sizeof(Settings.user_password));
    Response_P(S_JSON_COMMAND_SVALUE, XdrvMailbox.command, Settings.user_password);
  } else {
    Response_P(S_JSON_COMMAND_ASTERISK, XdrvMailbox.command);
  }
}
